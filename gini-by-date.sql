WITH
  traces_in AS (  
                            ...
                            WHERE   (DATE(block_timestamp) <= DATE_ADD('2016-01-01', INTERVAL 50 DAY ) )
            ),
                        
  blocks_in AS (
                            SELECT * 
                            FROM  `bigquery-public-data.crypto_ethereum.blocks` 
                            WHERE   (DATE(timestamp      ) <= DATE_ADD('2016-01-01', INTERVAL 50 DAY ) )
            ),
  ...
  calendar AS (
                            ...
                            WHERE   (    date              <= DATE_ADD('2016-01-01', INTERVAL 50 DAY ) )
              ),
